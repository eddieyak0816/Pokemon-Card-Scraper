...
def process_set(session, set_obj, download_images=False):
    ...
    try:
        cards = fetch_json(session, cards_url)
    except Exception as e:
        return {"csv": csv_filename, "set_name": set_name, "cards": 0, "error": str(e)}
    ...
    for i, card in enumerate(cards):
        ...
        if download_images and large:
            download_image(session, large, os.path.join(img_dir, img_name))
        elif download_images and small:
            download_image(session, small, os.path.join(img_dir, img_name))
    ...
    try:
        ...
        return {"csv": csv_filename, "set_name": set_name, "cards": len(rows), "error": str(e)}

    return {"csv": csv_filename, "set_name": set_name, "cards": len(rows)}
...
def main():
    ...
    sets = fetch_json(session, sets_url)

    # concurrency settings
    default_workers = (os.cpu_count() or 4) * 4
    max_workers = int(os.getenv('MAX_WORKERS', default_workers))
    download_images = os.getenv('DOWNLOAD_IMAGES', '0') == '1'
    sleep_between = float(os.getenv('SLEEP_BETWEEN_SETS', '0'))

    results = []

    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = {ex.submit(process_set, session, s, download_images): s for s in sets}
        for fut in as_completed(futures):
            try:
                res = fut.result()
                results.append(res)
                print(f"Completed: {res.get('set_name')} -> {res.get('csv')} ({res.get('cards')} cards)")
            except Exception as e:
                s = futures[fut]
                print(f"Error processing set {s.get('name')}: {e}")
            if sleep_between:
                sleep(sleep_between)
    ...
    try:
        with open(artifact_path, "w", encoding='utf-8') as af:
            json.dump({"new_csvs": results}, af, ensure_ascii=False, indent=2)
